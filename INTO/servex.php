	...
	if (empty($username)) {
		...
		array_push($errors,"Password is required");
			...
			if (count($errors) ==0) {
				...
				$sql = "INSERT INTO head (username, email, password) VALUES('$username', '$email', '$password')";
				mysqli_query($db,$sql);
				 $_SESSION['username'] = $username;
				        $_SESSION['success'] = "You are now logged in";
				        header('location: head.php'); // redirect to home page
	...
	if (empty($username)) {
		array_push($errors,"Username is required");
			...
			if (empty($password)) {
				...
				if (count($errors) == 0 ) {
				    $query = "SELECT * FROM head WHERE username='$username' AND password='$password'";
